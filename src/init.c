...
void chargement_parametres() //Cette fonction � pour but de r�cup�rer les contr�les sauvegard�s; Si jamais des contr�les sont perdu ou in�xistants, la fonction remet/met par d�fault des contr�les
{
    ...
    fscanf(f_parametres,"-GRAPHISMES-\nMode : %d\nResolution : %d\nFps : %d\nAfficherFps : %d\n-INTERFACE-\nEchelle : %d\n-AUDIO-\nMusique : %d\nEffets : %d\nDialogues : %d\nArrierePlan :%d\n-C-O-N-T-R-O-L-E-S-\n_JOUEUR1_\nAvancer : %d\nReculer : %d\nSauter : : %d\nAttaquer : %d\n_JOUEUR2_\nAvancer : %d\nReculer : %d\nSauter : : %d\nAttaquer : %d\n_JOUEUR3_\nAvancer : %d\nReculer : %d\nSauter : %d\nAttaquer : %d\n_JOUEUR4_\nAvancer : %d\nReculer : %d\nSauter : %d\nAttaquer : %d\n", &jeu_parametres.mode, &jeu_parametres.resolution, &jeu_parametres.fps, &jeu_parametres.affichFPS, &jeu_parametres.echelle, &jeu_parametres.musique,  &jeu_parametres.effets, &jeu_parametres.dialogues, &jeu_parametres.arriere_plan,   &jeu_parametres.controles[0][0],   &jeu_parametres.controles[0][1], &jeu_parametres.controles[0][2], &jeu_parametres.controles[0][3],    &jeu_parametres.controles[1][0],    &jeu_parametres.controles[1][1], &jeu_parametres.controles[1][2], &jeu_parametres.controles[1][3],     &jeu_parametres.controles[2][0],     &jeu_parametres.controles[2][1], &jeu_parametres.controles[2][2], &jeu_parametres.controles[2][3],      &jeu_parametres.controles[3][0],      &jeu_parametres.controles[3][1], &jeu_parametres.controles[3][2], &jeu_parametres.controles[3][3]);
    fclose(f_parametres);
}
...
void write_all_varia()
{
    ...
    fprintf(struct_jeu.debug, "\n\nEchelle : %d, Musique : %d, Effets : %d, Dialogues : %d, Arrière Plan : %d\nJoueur 1 : Avancer : %d, Reculer : %d, Sauter : %d, Attaquer : %d\nJoueur 2 : Avancer : %d, Reculer : %d, Sauter : %d, Attaquer : %d\nJoueur 3 : Avancer : %d, Reculer : %d, Sauter : %d, Attaquer : %d\nJoueur 4 : Avancer : %d, Reculer : %d, Sauter : %d, Attaquer : %d\nPage : %d\n\n",
            jeu_parametres.echelle,jeu_parametres.musique, jeu_parametres.effets, jeu_parametres.dialogues, jeu_parametres.arriere_plan,   jeu_parametres.controles[0][0],   jeu_parametres.controles[0][1], jeu_parametres.controles[0][2], jeu_parametres.controles[0][3],    jeu_parametres.controles[1][0],    jeu_parametres.controles[1][1], jeu_parametres.controles[1][2], jeu_parametres.controles[1][3],     jeu_parametres.controles[2][0],     jeu_parametres.controles[2][1], jeu_parametres.controles[2][2], jeu_parametres.controles[2][3],      jeu_parametres.controles[3][0],      jeu_parametres.controles[3][1], jeu_parametres.controles[3][2], jeu_parametres.controles[3][3],jeu_parametres.page);
